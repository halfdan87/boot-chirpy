...
const getAllChirpsInAscendingOrder = `-- name: GetAllChirpsInAscendingOrder :many
SELECT id, user_id, body, created_at, updated_at FROM chirps ORDER BY created_at ASC
`

func (q *Queries) GetAllChirpsInAscendingOrder(ctx context.Context) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpsInAscendingOrder)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
